...
function addPurchasePopup(data, time){
  ...
  // build popup html to inject int oalert
  ...
  //desktop max popup count
  var popupMax = maxPopupCount;
  
  //mobile position and count
  var screenW = document.documentElement.clientWidth;
  if(screenW < 767){
    popupMax = mobileMaxPopupCount;
    position = 'top-left';
  }
  ...
  //set time
  if(!time) time = 0;
  //dismiss if que is full
  if(queue.length >= popupMax){
    queue[0].dismiss();
  }
  ...
  //dismiss event to remove from queue
  message.ondismiss = function(){
    queue = queue.filter(item => item !== message);
  };
}

// return flag icon for country
function getFlag(countryCode, countryName){
  ...
  return c ? 'revamp/images/flags/' + c.country_code + '.png' : undefined;
}
...
// get product icon from product name
function getMemberIcon(membership){
  switch (membership.toLowerCase()) {

    case 'champion':
      return 'revamp/images/member-icons/champion_icon.svg';
    case 'hero':
      return 'revamp/images/member-icons/hero_icon.svg';
    case 'the real world':
      return 'revamp/images/member-icons/member_icon.svg';
    default:
      return '';
  }
  return '';
